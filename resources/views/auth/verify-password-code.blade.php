...
<div class="bg-black min-h-screen">
    <div class="relative min-h-screen flex items-center justify-center py-10 px-4 overflow-hidden">
        ...
        >
            ...
            <form method="POST" action="{{ route('password.verify-code.submit') }}" class="space-y-4" data-livewire-form data-livewire-html>
                @csrf

                <div class="space-y-1">
                    <label class="text-[10px] uppercase tracking-widest text-white font-bold">Reset Code</label>
                    <input
                        name="code"
                        inputmode="numeric"
                        pattern="[0-9]{6}"
            ...
            <p class="text-gray-500 mt-5 text-xs text-center leading-relaxed">
                The code expires in 10 minutes. Request a new code if it expires.
            </p>
        </div>
