    ...
    <div class="px-10 pb-10 space-y-4">
        ...
        <div class="bg-[#1a1a1a] border border-gray-800 rounded-2xl overflow-hidden flex items-center gap-5 p-4 hover:border-gray-600 transition-all duration-200">
            ...
            <div class="flex-1 min-w-0">
                ...
                <div class="flex flex-wrap gap-x-5 gap-y-1 text-gray-400 text-xs mb-3">
                    ...
                    <span class="flex items-center gap-1.5">
                        ...
                        {{ \Carbon\Carbon::parse($cart->start_date)->format('M j') }} – {{ \Carbon\Carbon::parse($cart->end_date)->format('M j, Y') }}
                    </span>
                    {{-- Fuel --}}
                    <span class="flex items-center gap-1.5">
                        <svg class="w-3.5 h-3.5 text-gray-500 shrink-0" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24">
            ...
            <div class="flex flex-col items-end gap-2 shrink-0">
                ...
                {{-- Checkout --}}
                <form method="POST" action="/cart/checkout/{{ $cart->id }}">
                    @csrf
                    <button type="submit"
                ...
                <form method="POST" action="/cart/remove/{{ $cart->id }}">
                    ...
                    @method('DELETE')
                    <button type="submit"
                            class="bg-[#2a2a2a] hover:bg-red-500 border border-gray-700 hover:border-red-500 text-gray-400 hover:text-white text-xs font-bold px-5 py-2 rounded-full transition-all duration-200 w-32 text-center">
                        Remove
                    </button>
                </form>
            </div>
