<div class="{{ $activeContact ? 'flex' : 'hidden lg:flex' }} min-w-0 flex-1 rounded-2xl bg-[#1a1a1a] border-white/5 shadow-2xl flex-col overflow-hidden relative lg:rounded-3xl lg:border">
    ...
    @if($activeContact)
        ...
        <div class="message-chat-header p-3 lg:p-6 border-b border-white/5 bg-[#1a1a1a]/80 backdrop-blur-md flex items-center justify-between gap-2 lg:gap-3">
            ...
            class="inline-flex min-w-0 flex-1 items-center gap-3 lg:gap-4 border border-transparent hover:border-white/30 rounded-xl px-1 py-1.5 lg:px-3 lg:py-2 lg:-mx-3 transition-all duration-300">
                <div class="w-10 h-10 lg:w-11 lg:h-11 bg-gray-800 rounded-full border border-white/10 overflow-hidden shrink-0">
                ...
                <div class="min-w-0">
                    <h2 class="truncate text-sm lg:text-base font-black uppercase tracking-tighter text-white">{{ $activeContact->username }}</h2>
        ...
        <div
            ...
            class="flex-1 overflow-y-auto p-3 lg:p-8 flex flex-col-reverse custom-scrollbar bg-[url('https://www.transparenttextures.com/patterns/dark-matter.png')]"
        >
            @forelse($messages->reverse() as $message)
                <div data-message-id="{{ $message->id }}" class="flex min-w-0 flex-col {{ $message->sender_id == auth()->id() ? 'items-end ml-auto' : 'items-start' }} max-w-[84%] lg:max-w-[70%] mt-4 lg:mt-6">
                    <div class="max-w-full px-4 py-3 lg:p-4 rounded-2xl shadow-md {{ $message->sender_id == auth()->id() ? 'bg-lime-400 text-black rounded-tr-none' : 'bg-[#242424] text-gray-300 rounded-tl-none border border-white/5' }}">
                        <p class="text-sm leading-relaxed break-words [overflow-wrap:anywhere]">{{ $message->body }}</p>
                    </div>
        ...
        >
            ...
            <div class="p-3 lg:p-6 bg-[#1a1a1a] border-t border-white/5" data-chat-form-wrap>
                <form id="chat-form" action="{{ route('messages.store') }}" method="POST" class="flex items-center gap-2 lg:gap-3">
                    @csrf
