...
<div class="bg-[#121212] min-h-screen flex items-center justify-center px-4 py-10">
    ...
    >
        <div class="text-center mb-6">
            <div class="mx-auto mb-5 flex h-12 w-12 items-center justify-center rounded-xl bg-yellow-400/10 text-yellow-400">
                <i class="fa-solid fa-envelope-circle-check text-xl"></i>
            </div>

            <p class="text-yellow-400 text-xs font-black uppercase tracking-[0.22em] mb-3">
                Verify new email
            </p>
        ...
        <form method="POST" action="{{ route('profile.email.verify-code.submit') }}" class="space-y-4" data-livewire-form>
            @csrf
            ...
            <div class="space-y-1">
                ...
                <input
                    ...
                    focus:outline-none focus:ring-2 {{ $errors->has('code') ? 'focus:ring-red-500 border-red-500' : 'focus:ring-yellow-400 focus:border-yellow-400/60' }}
                    transition"
                >
                @error('code')
            ...
            <button
                class="w-full py-3 rounded-md font-black text-black bg-yellow-400 uppercase tracking-wide
